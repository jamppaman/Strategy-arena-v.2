            ...
            for (int i = 0; i < attriList.arraySize; i++)
            {
                ...
                if (AttributeState != null)
                {
                    ...
                    switch (state)
                    {
                        ...
                        case SpellCreator.SpellAttribute.AttributeType.manipulation:
                            SerializedProperty teleportToTarget = attriRefe.FindPropertyRelative("teleportToTarget");
                            SerializedProperty chagePlaceWithTarget = attriRefe.FindPropertyRelative("chagePlaceWithTarget");
                            SerializedProperty moveCloserToTarget = attriRefe.FindPropertyRelative("moveCloserToTarget");
                            SerializedProperty moveAwayFromTarget = attriRefe.FindPropertyRelative("moveAwayFromTarget");
                            EditorGUILayout.PropertyField(teleportToTarget);
                            EditorGUILayout.PropertyField(chagePlaceWithTarget);
                            EditorGUILayout.PropertyField(moveCloserToTarget);
                            EditorGUILayout.PropertyField(moveAwayFromTarget);
                            break;

                        case SpellCreator.SpellAttribute.AttributeType.pull:
                            SerializedProperty spellPushback = attriRefe.FindPropertyRelative("spellPushback");
                            SerializedProperty mySpellPushType = attriRefe.FindPropertyRelative("mySpellPushType");
                            EditorGUILayout.PropertyField(mySpellPushType);
                            EditorGUILayout.PropertyField(spellPushback);
                            break;

                        case SpellCreator.SpellAttribute.AttributeType.push:
                            SerializedProperty spellPull = attriRefe.FindPropertyRelative("spellPull");
                            SerializedProperty mySpellPullType = attriRefe.FindPropertyRelative("mySpellPullType");
                            EditorGUILayout.PropertyField(mySpellPullType);
                            EditorGUILayout.PropertyField(spellPull);
